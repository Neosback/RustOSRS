...
uniform highp usampler2D u_playerDataTexture;
...
int applyHslOverride(int hsl, vec4 overrideValue) {
    if (overrideValue.w <= 0.0) return hsl;
...
Vertex decodeVertex(uvec3 packed, vec4 actorHslOverride) {
    ...
    float v = unpackFloat11(v0 & 0x7fffu);

    float y = -float(int(v1 & 0x7fffu) - 0x4000);
    int hsl = int((v1 >> 15u) & 0xffffu);
    float isTextured = float((v1 >> 31u) & 1u);
    uint textureId =
        uint(float((hsl >> 7) | int(((v2 >> 5u) & 1u) << 9u)) * isTextured);
...
float fogFactorOsrs(vec2 playerOffset) {
    ...
    float distance = sdRoundedBox(playerOffset, vec2(fogEnd), rounding);
...
PlayerInfo decodePlayerInfo(int offset) {
    ...
    uvec4 data = texelFetch(u_playerDataTexture, dataTexCoord(baseTexel), 0);
    uvec4 data1 = texelFetch(u_playerDataTexture, dataTexCoord(baseTexel + 1), 0);
...
void main() {
    ...
    vec4 localPos = vec4(vertex.pos, 1.0)
        * rotationY(float(playerInfo.rotation) * RS_TO_RADIANS)
        + vec4(playerInfo.tilePos.x, 0.0, playerInfo.tilePos.y, 0.0);
    ...
    float fog = fogFactorOsrs(localPos.xz - u_playerPos);
    v_fogAmount = loadAlpha < 1.0
        ? max(1.0 - loadAlpha, fog)
        : fog;
    ...
    gl_Position.z =
        depthLayerClipPos.z * gl_Position.w / depthLayerClipPos.w;
